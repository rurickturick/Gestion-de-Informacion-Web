...
class DBHelper {
   ...
   // Añade un usuario
	function add_user($usuario, $nombre, $apellidos, $dni, $correo, $password, $sal, $rol)
	{
	...
	function edit_user($usuario, $nombre, $apellidos, $dni, $correo, $password,$sal, $rol){
        $coleccion= $this->DB->usuarios;
		$nuevosdatos= array("usuario"=>$usuario,"nombre"=>$nombre,"apellidos"=>$apellidos, "dni"=>$dni,"correo"=>$correo,"password"=>$password,"sal"=>$sal, "rol"=>$rol);
	...
	{
	    $coleccion= $this->DB->usuarios;
		//Obtengo la sal + el password q me dan y confirmo que es valido.
		$sal = $this->get_sal($usuario);
		$password = $password.$sal;
	...
	function getValoraciones( $idTeatro,$nombre_obra,$ordenar)
	{
	  $coleccion= $this->DB->valoraciones;
	  $query=array("id_teatro"=>$idTeatro,"nombre_obra"=>$nombre_obra);
	   if($ordenar==0){
	  $consulta=$coleccion->find($query)->sort(array("valoracion"=>1,"user_dni"=>1));
	  }
	  else{
	  $consulta=$coleccion->find($query)->sort(array("fecha"=>1, "user_dni"=>1));
	  }
	  return $consulta;
	}
